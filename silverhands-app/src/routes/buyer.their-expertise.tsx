...
export const Route = createFileRoute("/buyer/their-expertise")({
  head: () => ({
    meta: [
      { title: "Find an expert — SilverHands" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Find an expert — SilverHands" },
      {
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-border bg-card p-6 shadow-sm transition-shadow hover:shadow-md">
      ...
      <Button className="mt-5 h-12 w-full text-base" asChild>
        <Link to="/buyer/expert/$expertId" params={{ expertId: expert.id }}>
          View profile
        </Link>
      </Button>
    </article>
