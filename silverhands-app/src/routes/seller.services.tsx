...
import { promote, unpromote, usePromotedIds, SERVICE_AD_COST } from "@/lib/ads";
    ...
    <div className="space-y-14">
      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {services.slice(0, 3).map((s) => {
          const sponsored = promotedIds.has(s.id);
          return (
              ...
              <div className="space-y-2 p-5">
                ...
                {sponsored ? (
                  <Button
                    ...
                    onClick={() => {
                      unpromote(s.id);
                      toast("Promotion stopped", { description: s.title });
                    }}
                  >
                    <Sparkles className="text-accent" /> Sponsored — stop
                  </Button>
                ) : (
                  <Button
                    type="button"
                    ...
                    className="w-full"
                    onClick={() => {
                      promote(s.id, "service", SERVICE_AD_COST);
                      toast.success("Listing promoted!", {
                        description: `${s.title} will now appear in Sponsored placements — ${inr(SERVICE_AD_COST)}/week.`,
                      });
                    }}
                  >
                    <Sparkles /> Promote — {inr(SERVICE_AD_COST)}/week
                  </Button>
                )}
              </div>
            </div>
