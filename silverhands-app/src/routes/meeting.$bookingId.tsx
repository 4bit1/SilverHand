...
export const Route = createFileRoute("/meeting/$bookingId")({
  ...
  head: () => ({
    meta: [
      { title: "Your session — SilverHands" },
      {
        name: "description",
        content: "Join your booked SilverHands conversation at the scheduled time.",
      },
      { property: "og:title", content: "Your session — SilverHands" },
      {
...
function Meeting() {
  ...
  const now = useNow();
  const shSession = useSession();
  const shSessionRef = useRef(shSession);
  shSessionRef.current = shSession;
  ...
  const handleLeave = useCallback(() => {
    call.leave();
    // Prefer the origin route captured before navigating into the meeting
    // (passed as the `from` search param by the Join / View links).
    if (from) {
      void navigate({ to: from });
      return;
    }
    // Fallback: infer from the session role when `from` is absent
    // (e.g. a direct link shared externally).
    let currentRole = shSessionRef.current?.role;
    if (!currentRole) {
      try {
