...
import { promote, unpromote, usePromotedIds, PRODUCT_AD_COST } from "@/lib/ads";
  ...
  return (
    <div className="space-y-14">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((p) => {
          const sponsored = promotedIds.has(p.id);
          return (
              ...
              <div className="space-y-2 p-5">
                ...
                {sponsored ? (
                  <Button
                    ...
                    onClick={() => {
                      unpromote(p.id);
                      toast("Promotion stopped", { description: p.name });
                    }}
                  >
                  ...
                  <Button
                    ...
                    className="w-full"
                    onClick={() => {
                      promote(p.id, "product", PRODUCT_AD_COST);
                      toast.success("Listing promoted!", {
                        description: `${p.name} will now appear in Sponsored placements — ${inr(PRODUCT_AD_COST)}/week.`,
                      });
                    }}
                  >
                    <Sparkles /> Promote — {inr(PRODUCT_AD_COST)}/week
                  </Button>
                )}
              </div>
            </div>
